class Player():
	...
	def print_stats(self):
		...
		print("Played: %i, Won: %i, Lost: %i, Tied: %i" % (self.played, self.won, self.lost, self.tied))
		print("-----------")
...
class Board:
    ...
    def __init__(self):
        self.grid = [[0, 0, 0], [0, 0, 0], [0, 0, 0]]  # Initializes a empty grid
        self.player = 1
    ...
    def __col_match(self):
        t_grid = zip(*self.grid)  # Transposes grid to make row matching easier
        for row in t_grid:
            if row[0] != 0 and row[0] == row[1] == row[2]:
                return True
        return False
...
class Game:
    ...
    def start_game(self):
        ...
        print("\nAlright, " + self.players[0].name + ", let's go! You will play with 'X's. ")
    ...
    def play_game(self):
        ...
        while not self.board.has_won():  
            ...
            square = input(self.players[current_player].name + ", choose a square to play (e.g. A3): ")

			# Plays the square if possible
            while not self.board.player_turn(square, self.players[current_player].symbol):
				# Gets angry when user passes an invalid input
                square = input("Please choose an unfilled square between A1 and C3: ")  
    ...
    def play_again_prompt(self, tie, current_player):
        if not tie:
            return ("Congratulations, " + self.players[current_player].name + ", you win! Would you like to play again? (Y/N) ")
